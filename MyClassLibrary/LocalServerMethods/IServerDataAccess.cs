...
namespace MyClassLibrary.LocalServerMethods
{
    public interface IServerDataAccess
    {
        ...
        /// </summary>
        public Task<List<T>> GetFromServer<T>(List<Guid>? ids = null) where T : LocalServerIdentityUpdate;
        ...
        /// </summary>
        public Task<(List<T> changesFromServer ,DateTime lastUpdatedOnServer)> GetChangesFromServer<T>(DateTime LastSyncDate) where T : LocalServerIdentityUpdate;


        /// <summary>
        /// Saves Objects Data that inherit from LocalServerIdentity into Server Storage. Pass back UpdatedOnServer dat to objects.
        /// </summary>
        public Task<DateTime> SaveToServer<T>(List<T> updates) where T : LocalServerIdentityUpdate;

        /// <summary>
        /// Adds the corresponding Conflict Id (value) to all objects inhereting from local server identity with Id = (key)
        /// </summary>
        public Task SaveConflictIdsToServer<T>(List<Conflict> conflicts) where T : LocalServerIdentityUpdate;

        /// <summary>
        /// Permanently deletes the objects inheriting from LocalServerIdentiy passed in from server storage.
        /// </summary>
        public Task DeleteFromServer<T>(List<T> updates) where T : LocalServerIdentityUpdate;
