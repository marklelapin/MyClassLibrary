...
namespace MyClassLibrary.Tests.LocalServerMethods.Tests
{
    public class TestContent
    {
        ...
        {
            DateTime DateNow = createdDate ?? DateTime.Now;
            ...
            testSetupType = testSetupType ?? "DataAccessTesting";

            List<Guid> draftIds = OverrideIds ?? GeneratDraftIds(25);
            draftIds.Sort((x,y)=>x.CompareTo(y));

            
            switch(testSetupType)
            {                
                case "DataAccessTesting":
